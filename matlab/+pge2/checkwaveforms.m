...
arg.plot = true;   % plot every segment instance, continue on keyboard press
...
while n < ceq.nMax % & cnt < 2
    ...
    for iax = 1:length(ax)
        ...
        if arg.plot
            ...
            legend('seq', 'pge2');
            xlabel('time (sec)');
            ylabel(sprintf('%s (G/cm)', ax{iax}), 'Rotation', 0);
            if iax == 1
    ...
    if isempty(arg.row)
        input('Press Enter key to continue to next segment ', "s");
        n = n2 + 1;
    end
end
