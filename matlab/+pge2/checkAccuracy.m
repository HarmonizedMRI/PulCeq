...
% and/or the WTools simulator output (.xml files), 
...
while n < ceq.nMax & cnt < 2
    ...
    plot(d(1).time(1:end-3)/1e6 - sysGE.segment_dead_time, d(1).value(1:end-3), 'rx-');
    plot(S.gx.t - sysGE.segment_dead_time, S.gx.signal, 'go');
